...
```bash
ssh-keygen -t ed25519 -C "287562898+rajkumar-saini@users.noreply.github.com" -f ~/.ssh/id_ed25519 # Generate SSH key
# Start SSH agent
eval "$(ssh-agent -s)"
ssh-add ~/.ssh/id_ed25519
cat ~/.ssh/id_ed25519.pub # Display public key (add this to GitHub/GitLab)
```
```bash
ssh -T git@github.com # Verification Check
```
...
```
```bash
tmux # Start tmux session for long-running builds
```
---
...
```
#### Bringup Device Tree & Setup Signing Keys

- [**Lineage OS**](./Guides/LineageOS.md)
- [**Project Infinity X**](./Guides/ProjectInfinity-X.md)
- [**Rising Revived OSS**](./Guides/RisingRevivedOSS.md)
---
## 🔨 Build Process
...
ls -la vendor/bcr/
df -h # Check available disk space (minimum 200GB free)
...
**Flash Instructions**
- **Method 1: ADB Sideload via Recovery (Recommended)**
```bash
